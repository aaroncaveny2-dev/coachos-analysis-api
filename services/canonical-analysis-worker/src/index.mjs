...
async function loadGame(client, gameId) {
  ...
  if (!result.rows[0]) throw new Error(`game not found: ${gameId}`);
  if (!result.rows[0].pgn) throw new Error(`game has no PGN: ${gameId}`);
...
async function processJob(job) {
  ...
  try {
    ...
    const history = pgnChess.history({ verbose: true });
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (
        ...
        completed++;
        replay.move(move);
        await updateQueueProgress(client, job.queue_id, completed, total);
        continue;
      }
      ...
      const evalBest = before.cp;
      const cpLoss = computeCpLossMover({
      ...
      await writeCanonicalRow(client, row, {
        ...
        engineMetadata: makeEngineMetadata({ depth: before.depth || STOCKFISH_DEPTH })
      });
